...
#SBATCH --export=NONE
#SBATCH --array=1001-9236
#SBATCH --job-name=MAKE_EXAMPLES_CALL_X1n
...
X_WINDOWS='regions/Ga_1n_windows.bed'
...
if [ ! -d "$WD/examples_X1n/$SAMPLE" ]; then
   mkdir -p $WD/examples_X1n/$SAMPLE
fi

## make gvcf TF records output dir

if [ ! -d "$WD/GVCF_TFRECORDS_X1n/$SAMPLE" ]; then
   mkdir -p $WD/GVCF_TFRECORDS_X1n/$SAMPLE
fi
...
--ref /wd/$REF \
--reads /wd/$BAM_PATH \
--examples /wd/examples_X1n/$SAMPLE/${SAMPLE}_examples_positional.tfrecord@${N_SHARDS}.gz \
--gvcf /wd/GVCF_TFRECORDS_X1n/$SAMPLE/${SAMPLE}_gvcf.tfrecord@${N_SHARDS}.gz \
--regions /wd/$X_WINDOWS \
