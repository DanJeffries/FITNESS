...
#SBATCH --mem=250G
#SBATCH --export=NONE
#SBATCH --job-name=SHUFFLE_global_validate
...
python3 ${SHUFFLE_SCRIPT_DIR}/shuffle_tfrecords_beam.py \
  ...
  --input_pattern_list="${EXAMPLES_DIR}/validate_all/*_validation_examples.tfrecord-?????-of-00020" \
  --output_pattern_prefix="${OUTPUT_DIR}/All_samples_validation_examples.shuffled" \
  --output_dataset_name="Shuffle_global" \
  --output_dataset_config_pbtxt="${OUTPUT_DIR}/All_samples_validation_examples.dataset_config.pbtxt" \
