...
MODEL_SAMPLE_MAP=/storage/homefs/dj20y461/Stickleback/G_aculeatus/FITNESS/code/DV_training/22_mendelian_evaluation_scripts/Step_1/test_call_dirs.txt
TESTCALL_DIR=$(sed -n "${SLURM_ARRAY_TASK_ID}p" < $MODEL_SAMPLE_MAP)

CROSS="FG"

## Make the combined pedigree VCF

bcftools merge -O u \
	       $WD/$TESTCALL_DIR/${CROSS}_male_par*vcf.gz \
	       $WD/$TESTCALL_DIR/${CROSS}_fem_par*vcf.gz \
	       $WD/$TESTCALL_DIR/${CROSS}_male_1*vcf.gz |\
bcftools view  -e "N_MISSING > 0" \
	       -O z \
	       > $WD/$TESTCALL_DIR/${CROSS}_pedigree.vcf.gz 
...
$RTG mendelian  -i $WD/$TESTCALL_DIR/${CROSS}_pedigree.vcf.gz \
		-t $SDF_OUT \
		-o $WD/$TESTCALL_DIR/${CROSS}_pedigree.mend_eval.vcf \
		--pedigree $PED_FILE

bgzip $WD/$TESTCALL_DIR/${CROSS}_pedigree.mend_eval.vcf
tabix $WD/$TESTCALL_DIR/${CROSS}_pedigree.mend_eval.vcf.gz
