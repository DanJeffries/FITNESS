...
#SBATCH --mem=990G
#SBATCH --export=NONE
#SBATCH --job-name=SHUFFLE_train_SNPS_ONLY
...
OUTPUT_DIR=${WD}/examples_shuffled/train_SNPS_ONLY
...
EXAMPLE_SUBSET_DIR=${WD}/examples/train_SNPS_ONLY/test_subset
...
python3 ${SHUFFLE_SCRIPT_DIR}/shuffle_tfrecords_beam.py \
  ...
  --output_pattern_prefix="${OUTPUT_DIR}/examples_shuffled" \
  --output_dataset_name="Shuffle_global" \
  --output_dataset_config_pbtxt="${OUTPUT_DIR}/examples_shuffled_config.pbtxt" \
