...
#SBATCH --export=NONE
#SBATCH --array=3  # start from 2 because of the header in the params file. 
#SBATCH --job-name=TRAIN_GRIDSEARCH_STEP_4
...
STEP_PARAMS="/storage/homefs/dj20y461/Stickleback/G_aculeatus/FITNESS/code/DV_training/19_training_scripts/Step_4/step_parameters.txt"
...
--config.tune_dataset_pbtxt="/home/examples_shuffled_NEW/tune/GRIDSEARCH_tuning_subset/tune_examples.config.pbtxt" \
--config.num_epochs=2 \
--config.learning_rate=$LR \
--config.learning_rate_decay_rate=$LRD \
--config.learning_rate_num_epochs_per_decay=0.5 \
