...
OUTPUT_DIR=${WD}/examples_shuffled_NEW/train/training_subset
...
EXAMPLE_SUBSET_DIR=$EXAMPLES_DIR/subset
...
for i in $(cat $SHUFFLED_EXAMPLES_FILE | awk "NR % 5 == 1"); do mv ${EXAMPLES_DIR}/${i}* $EXAMPLE_SUBSET_DIR; done
...
python3 ${SHUFFLE_SCRIPT_DIR}/shuffle_tfrecords_beam.py \
  ...
  --output_pattern_prefix="${OUTPUT_DIR}/examples_shuf2" \
  --output_dataset_name="Shuffle_global" \
  --output_dataset_config_pbtxt="${OUTPUT_DIR}/examples_shuf2_config.pbtxt" \
